		...
		for thres in  0.7; do
			for max_epoch in 5; do 
				...
				#for model_list in 'unixcoder,unixcoder' ; do
					...
					elif [ "$model_list" == 'codet5p,unixcoder' ]; then
						lr=("4e-4 2e-4")
					fi
					for seed in 45 ; do   
						...
						python3 panel_main_original_copy.py ${nshot} "${model_list}" "${lr}" ${thres} ${max_epoch} ../data/jointmatch/${language} ${seed} #> ./jointmatch_log/${language}/${nshot}_${model_list}_${thres}_${language}_${max_epoch}_${seed}.log
					done
				done
			done
		done
