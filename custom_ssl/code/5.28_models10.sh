                ...
                for model in 'codebert' 'codet5p' 'graphcodebert' 'unixcoder' ; do
                    ...
                    for seed in 42; do
                        if [ "${model}" = "codesage" ]; then
                            lr_values='3e-5'  # Two learning rates for codesage
                        elif [ "${model}" = "codebert" ]; then
                            lr_values='3e-5'  # One learning rate for codebert
                        elif [ "${model}" = "codet5p" ]; then
                            lr_values='3e-5'  # One learning rate for codet5p
                        elif [ "${model}" = "graphcodebert" ]; then
                            lr_values='3e-5'  # One learning rate for graphcodebert
                        elif [ "${model}" = "unixcoder" ]; then
                            lr_values='2e-6'  # One learning rate for unixcoder
                        fi
