	...
	for language in java ; do
		for thres in  0.7; do
			for max_epoch in 20; do 
				for model_list in  "unixcoder,unixcoder" "codet5p,codet5p"; do
					...
					for seed in 42 43 45; do   
						echo "python3 panel_main_original.py ${nshot} '${model_list}' '${lr}' ${thres} ${max_epoch} ../data/jointmatch/${language} ${seed} > ./jointmatch_log/${language}/${nshot}_${model_list}_${thres}_${language}_${max_epoch}_${seed}_change_lr.log"
						python3 panel_main_original.py ${nshot} "${model_list}" "${lr}" ${thres} ${max_epoch} ../data/jointmatch/${language} ${seed} > ./jointmatch_log/${language}/${nshot}_${model_list}_${thres}_${language}_${max_epoch}_${seed}_change_lr.log
					done
				done
			done
		done
