...
# CSV 파일에서 데이터 로드
dataset = 'corcod'
# dataset = './CodeComplex/java_extended_data'
data = pd.read_json("/home/sunny5574/SALNet/CodeData/corcod.index.jsonl",lines=True)
...
# 데이터프레임을 CSV 파일로 저장
train_data.to_csv(f'./{dataset}/train.csv', index=False)
valid_data.to_csv(f'./{dataset}/dev.csv', index=False)
test_data.to_csv(f'./{dataset}/test.csv', index=True)
