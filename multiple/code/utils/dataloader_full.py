...
def train_split(labels):
    ...
    for i in all_classes:
        ...
        train_labeled_idxs.extend(idxs)  # 모든 인덱스를 가져옵니다.
...
def get_dataloader(data_path, ul_data_path, n_labeled_per_class, bs, load_mode='semi'):
    ...
    train_labeled_idxs, train_unlabeled_idxs = train_split(labels)
    
    # code contest ul_data 로드
    new_unlabeled_df = pd.read_csv(os.path.join(ul_data_path, 'train.csv'))

    # train_u_df에 새로운 데이터셋 할당
    train_u_df = new_unlabeled_df.reset_index(drop=True)
    ...
    if load_mode == 'semi':
        ...
        train_dataset_u = SEMIDataset(train_u_df['content'].to_list(), train_u_df['synonym_aug'].to_list(), train_u_df['back_translation'])
