...
def get_dataloader_aug_v1(data_path, dataset, n_labeled_per_class, bs, load_mode='semi_SSL',  token = None):
    ...
    backtranslation_df = backtranslation_df[~backtranslation_df['content'].str.contains('ERROR')] #Error가 포함되어 있는 데이터는 제거
    ...
    forwhile_df = forwhile_df[~forwhile_df['content'].str.contains('ERROR')] #Error가 포함되어 있는 데이터는 제거
